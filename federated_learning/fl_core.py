...
class FL:
    def __init__(self, dataset_name, model_name, dd_type, num_clients, frac_clients,
                 ...
                 class_per_client=2, samples_per_class=250, rate_unbalance=1, alpha=1, source_class=None,trustScores=[]):
    ...
    """
    def choose_clients(self, min_dropout_rate, max_dropout_rate,trust_scores):
    # Generate a random dropout rate within the specified range
     dropout_rate = np.random.uniform(min_dropout_rate, max_dropout_rate)

    # Determine the number of clients to select after dropout
     num_selected_clients = max(int((1 - dropout_rate) * self.num_clients), 1)

     trust_scores_normalized = trust_scores / np.sum(trust_scores)

    # Randomly select clients to participate
     selected_clients = np.random.choice(range(self.num_clients), num_selected_clients, replace=False,p=trust_scores_normalized)

     return selected_clients, dropout_rate
    """
    def choose_clients(self, iteration):
    # Set the seed for reproducibility
     np.random.seed(iteration)
    
    # Define the dropout rate
     dropout_rate = np.random.randint(0, 61)  # Randomly choose dropout rate between 0 and 60
    
    # Determine the number of clients to select
     num_selected_clients = max(int((1 - dropout_rate / 100) * self.num_clients), 1)
    
    # Randomly select clients to participate
     selected_clients = np.random.choice(range(self.num_clients), num_selected_clients, replace=False)
    
    # Print selected clients for debugging
    ...
    """
    REWARD and PENALTY:
    def choose_clients(self, trust_scores):
     m = max(int(self.frac_clients * self.num_clients), 1)
     selected_clients = np.random.choice(range(self.num_clients), m, replace=False, p=trust_scores)
     return selected_clients
"""
    def update_trust_scores(self, selected_clients):
        decay_factor = 0.9  # Decay factor for trust scores
        trust_scores = self.trustScores  # Copy current trust scores
        trust_scores *= decay_factor  # Decay existing trust scores

        # Update trust scores based on client participation
        for client_idx in range(self.num_clients):
            if client_idx in selected_clients:
                trust_scores[client_idx] += 0.1  # Increase trust score for participating clients
            else:
                trust_scores[client_idx] -= 0.2  # Decrease trust score for dropout clients

            # Ensure trust scores are within the range [0, 1]
            trust_scores[client_idx] = max(0.1, min(1, trust_scores[client_idx]))
    ...
    def run_experiment(self, attack_type1='no_attack',attack_type2='no_attack', malicious_behavior_rate=0,
        ...
        last_grad_table = [None for _ in range(self.num_clients)]



        for epoch in tqdm(range(start_round, self.global_rounds)):
            self.trustScores = np.ones(self.num_clients)  # Initialize trust scores
            ...
            selected_clients , dropout_rate = self.choose_clients(0.1,0.5,self.trustScores)
            logger.info(f"Dropout rate for epoch {epoch}: {dropout_rate}")

            #self.trustScores = self.update_trust_scores(selected_clients)
            #selected_clients = self.choose_clients(epoch)
            ...
            if rule == 'median':
                ...
                scores , local_weights2 = dpfla.score(copy.deepcopy(simulation_model),
                ...
                #local_weights22 = [local_weights2[idx] for idx in benign_indices]
                ...
                #global_weights = average_weights(local_weights22, scores)
                    ...
                    for key in net_para:
                        ...
                        global_weights[key] = model_weights_mean[ current_idx:current_idx + length].reshape(net_para[key].shape)
                        current_idx += length
