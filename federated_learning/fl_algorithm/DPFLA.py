...
class DPFLA:
    ...
    def score(self, global_model, local_models, clients_types, selected_clients, p, w):
        ...
        bad_update_count = np.sum(np.logical_and(np.array(clients_types) == "Bad update", scores == 0))
        good_update_count = np.sum(np.logical_and(np.array(clients_types) == "Good update", scores == 1))
        ...
        predicted_labels = np.array(scores)
